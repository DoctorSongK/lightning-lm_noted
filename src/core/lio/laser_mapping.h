...
class LaserMapping {
   ...
   private:
    ...
    /// @brief 将传感器坐标系下的点转换到world
    /// @param pi 雷达坐标系下的某个点
    /// @param po 世界坐标系下的转换点
    /// 式中state_point_为世界坐标系下base_link坐标；offset_R_lidar是雷达相对base_link的相对偏移量
   ...
   private:
    ...
    std::vector<Vec4f> corr_pts_;                     // inlier pts 内点
    std::vector<Vec4f> corr_norm_;                    // inlier plane norms 内平面范数
    pcl::VoxelGrid<PointType> voxel_scan_;            // voxel filter for current scan

    std::vector<float> residuals_;           // point-to-plane residuals
    std::vector<bool> point_selected_surf_;  // selected points 用于存放是否可以生成平面
    std::vector<Vec4f> plane_coef_;          // plane coeffs 用于存放邻点生成平面的方程平面系数
    ...
    int scan_num_ = 0;
    int effect_feat_num_ = 0, frame_num_ = 0;  // 有效特征数量
