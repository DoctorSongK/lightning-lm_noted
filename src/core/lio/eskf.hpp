...
class ESKF {
   public:
    ...
    static constexpr int state_dim_ = NavState::dim;                // 状态维度（23）
    using StateVecType = NavState::VectState;                       // 状态向量类型（23行1列）
    using CovType = Eigen::Matrix<double, state_dim_, state_dim_>;  // 协方差矩阵（23 * 23）
